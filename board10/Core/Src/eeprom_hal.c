  ...
  switch (PageStatus0)
  {
    case ERASED:
      ...
      else if (PageStatus1 == RECEIVE_DATA) /* Page0 erased, Page1 receive */
      {
        ...
        /* Mark Page1 as valid */
         status = HAL_FLASH_Program(FLASH_TYPEPROGRAM_HALFWORD,PAGE1_BASE_ADDRESS, VALID_PAGE);
         if (status != HAL_OK)
                 {
                   return status;
                 }
...
static HAL_StatusTypeDef EE_Format(void)
{
  ...
  /* Set Page0 as valid page: Write VALID_PAGE at Page0 base address */
   status = HAL_FLASH_Program(FLASH_TYPEPROGRAM_HALFWORD,PAGE0_BASE_ADDRESS, VALID_PAGE);
   if (status != HAL_OK)
           {
             return status;
           }
...
static uint16_t EE_PageTransfer(uint16_t VirtAddress, uint16_t Data)
{
  ...
  /* Set the new Page status to RECEIVE_DATA status */
   status = HAL_FLASH_Program(FLASH_TYPEPROGRAM_HALFWORD,NewPageAddress, RECEIVE_DATA);
   if (status != HAL_OK)
           {
             return status;
           }
  ...
  /* Set new Page status to VALID_PAGE status */
   status = HAL_FLASH_Program(FLASH_TYPEPROGRAM_HALFWORD,NewPageAddress, VALID_PAGE);
   if (status != HAL_OK)
           {
             return status;
           }
